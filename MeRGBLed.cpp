...
{
#ifdef DEFALUT_LED_NUMBER
  setNumber(DEFALUT_LED_NUMBER);
#else
  setNumber(4);
#endif
}
...
void MeRGBLed::reset(uint8_t port,uint8_t port_mode)
{
  if(NATIVE_PIN == port_mode)
  {
...
bool MeRGBLed::setColor(uint8_t index, uint8_t red,uint8_t green,uint8_t blue)
{
  ...
  {
    return setColorAt(index,red,green,blue);
  } 
  return false;
}
